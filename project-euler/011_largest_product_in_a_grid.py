...
def check_horizontals(
    ...
    for row in grid:
        for window_start in range(grid_size - window_size):
            current_product = 1
...
def get_largest_grid_product(grid: list[list[int]], window_size: int) -> int:
    """
    Return the largest product of consecutive `window_size`
    elements in the grid in any direction
    """

    max_product = 0

    max_product = check_horizontals(grid, window_size, max_product)

    max_product = check_horizontals(
...
print(get_largest_grid_product(GRID, 4))
